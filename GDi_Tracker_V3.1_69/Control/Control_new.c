...
QEvt const CtrlRebootEvt= {CONTROL_REBOOT_SIG,0U,0U};
//-------------PRIVATE------------------------------- 

static QEvt const ModemSmsSendEvt=        {MODEM_SEND_SMS_SIG,0U,0U};
...
static QEvt const ControlUpdateEndEvt =   { CONTROL_UPDATE_END_SIG, 0U, 0U};
static QEvt const ControlUpdateStartEvt = { CONTROL_UPDATE_START_SIG, 0U, 0U};


static uint8_t fwu_buf[MAX_FWU_PACKET_SIZE];
static int CheckConfigUpdTimeout(void);
static int DiskIsNotEmpty(void);
static void SendEnable(CONTROL* const me);
static void UpdateOn(CONTROL* const me);
static void UpdateOff(CONTROL* const me);
static void UpdateDefer(CONTROL* const me);
...
static void OutDebugControlSprintf3( char const *,char const *,char const *,uint32_t,uint32_t,uint32_t);
#else
    ...
    #define OutDebugControlSprintf2(x1,x2,x3,x4) __nop()
		 #define OutDebugControlSprintf3(x1,x2,x3,x4,x5,x6) __nop()
#endif
...
static QState CtrlMemoryErrSuper11(CONTROL * const me, QEvt const * const e);
//----------------------------------------------------------------------
static QState CtrlUpdSuper12(CONTROL* const me, QEvt const * const e);//ok
static QState CtrlUpdStart13(CONTROL * const me, QEvt const * const e);//ok
static QState CtrlUpdReOpenConnection14(CONTROL * const me, QEvt const * const e);//ok
static QState CtrlUpdConf15(CONTROL * const me, QEvt const * const e);//ok
static QState CtrlUpdSupervisor16(CONTROL * const me, QEvt const * const e);
static QState CtrlUpdFwStart17(CONTROL * const me, QEvt const * const e);//ok
static QState CtrlUpdFwContinue18(CONTROL * const me, QEvt const * const e);//ok
...
QState CtrlWaitDiskReady2(CONTROL * const me, QEvt const * const e)
{
    switch (e->sig)
        ...
        case Q_ENTRY_SIG: 
            ...
            return Q_HANDLED();
        case CONTROL_DISK_READY_SIG: 
				{
					 conf.log.evt_counter.restart++;
					 conf.log.save=1;
           QACTIVE_POST(AO_Disk,&DiskSaveConfigEvt, me);
				}
					return Q_TRAN(&CtrlPackSuper3);
        case CONTROL_FLASH_ERROR_SIG: 
        case CONTROL_FRAM_ERROR_SIG:  return Q_TRAN(&CtrlMemoryErrSuper11);
        case Q_EXIT_SIG:  
				{
					//if( conf.log.updTryCounter>0)//if update allowed
					//{
						//SysInfo.msg_ptr.system ="TRACKER STARTED";
					if(conf.secur.send_reset_sms==1)QACTIVE_POST(AO_Modem,&ModemSmsSendEvt, me);
					//}
				}
					return Q_HANDLED();
        }
...
QState CtrlUpdSuper12(CONTROL* const me, QEvt const * const e)
{
    switch (e->sig)
        ...
        case Q_ENTRY_SIG:
            {
                AddStateHistory(12);
                me->send_tick_counter=0;
                me->send_enable=0;
							SysInfo.flag.fwu_processing=1;
            }
            return Q_HANDLED();
        case CONTROL_UPDATE_CONNECT_OK_SIG:
            ...
            return Q_HANDLED();     
        case Q_INIT_SIG:              return Q_TRAN(&CtrlUpdStart13);//update entry point
				case CONTROL_REBOOT_SIG:      return Q_TRAN(&CtrlSysRebootSuper22);
        case CONTROL_UPDATE_END_SIG:  return Q_TRAN(&CtrlPackSuper3);
        case CONTROL_FLASH_ERROR_SIG: 
        case CONTROL_FRAM_ERROR_SIG:  return Q_TRAN(&CtrlMemoryErrSuper11); 
        case Q_EXIT_SIG:  
				{
					SysInfo.flag.fwu_processing=0;
				}
					return Q_HANDLED();
        }
    return  Q_SUPER(&QHsm_top);
...
QState CtrlUpdStart13(CONTROL * const me, QEvt const * const e)
{
    switch (e->sig)
        {
        case Q_ENTRY_SIG:
            {
                ...
                me->upd_state=state_start;
                QTimeEvt_postIn(&me->TimeEvt, (QActive *)me,1); //kick start
            }
            return Q_HANDLED();
        case CONTROL_TIMEOUT_SIG: return Q_TRAN(&CtrlUpdSupervisor16); 
        case Q_EXIT_SIG: return Q_HANDLED();
        }
    return Q_SUPER(&CtrlUpdSuper12);
}

QState CtrlUpdSupervisor16(CONTROL * const me, QEvt const * const e)
{
    ...
    switch (e->sig)
        ...
        case CONTROL_TIMEOUT_SIG:
            switch (me->upd_state)
                ...
                case state_start:
                    {
                        OutDebugControl("CtrlUpdSupervisor16::state_start");
                    }
                    return Q_TRAN(&CtrlUpdReOpenConnection14); 
                case state_upd_conf: 
                    OutDebugControl("CtrlUpdSupervisor16::state_upd_conf");                          
                    me->reopen_connection_counter=0;
                    if (conf.secur.fw_ver_new!=0)
                        {
                        me->upd_state=state_upd_fw_start;
                        ret= Q_TRAN(&CtrlUpdFwStart17); 
                        }
                    else
                        {
                        UpdateOff(me);
                        ret= Q_TRAN(&CtrlUpdFinal19);
                        }
                    return ret;
                case state_upd_fw_start: 
                    {
                        OutDebugControl("CtrlUpdSupervisor16::state_upd_fw_start");   
                        me->upd_state=state_upd_fw_continue;
                    }
                    return Q_TRAN(&CtrlUpdFwContinue18); 
                case state_upd_fw_continue:
                    {
                        UpdateOn(me);//after reboot will be connected to service server to update config
											 QACTIVE_POST(AO_Control,&CtrlRebootEvt, me);
                    }
                    return Q_HANDLED();
                default:
                    {
                        OutDebugControl("CtrlUpdSupervisor16::UNKNOWN STATE!!!!");    
                    }
                    return Q_HANDLED();
                }
        case Q_EXIT_SIG:
...
QState CtrlUpdReOpenConnection14(CONTROL * const me, QEvt const * const e)
{
    ...
    switch (e->sig)
        ...
        case Q_ENTRY_SIG: 
            { 
                AddStateHistory(14);
                if (me->upd_state==state_start)
                    {
                    me->upd_state=state_upd_conf;
                    me->reopen_connection_counter=0;
                    QACTIVE_POST(AO_Modem,&ModemUpdStartEvt, me);
                    }
                OutDebugControlSprintf1("CTRL14 UPDATE reopen_counter=",me->reopen_connection_counter);
                if (++me->reopen_connection_counter>10)
                    {
                    restart=0;
                    QTimeEvt_postIn(&me->TimeEvt, (QActive *)me,400); //5*400=2000 ms final timeout
                    }
                else
                    {
                    restart=1;
                    QTimeEvt_postIn(&me->TimeEvt, (QActive *)me,2000); //5*2000=10 000 ms restart timeout
                    }
            }
            return Q_HANDLED();
        case CONTROL_UPDATE_CONNECT_OK_SIG:
            OutDebugControlSprintf2("CTRL14 UPDATE_CONNECT_OK_SIG upd_state=","reopen counter=",me->upd_state,me->reopen_connection_counter);
            switch (me->upd_state)
                {
                case state_upd_conf: return Q_TRAN(&CtrlUpdConf15); 
                case state_upd_fw_start: return Q_TRAN(&CtrlUpdFwStart17); 
                case state_upd_fw_continue: return Q_TRAN(&CtrlUpdFwContinue18);
                case state_start: return Q_HANDLED();//it will never happen
                }
        case CONTROL_TIMEOUT_SIG:
            {
                if (restart)
                    {
                    QACTIVE_POST(AO_Modem,&ModemUpdRestartEvt, me);
                    ret= Q_HANDLED();
                    }
                else
                    {
                    UpdateDefer(me);
                    ret=Q_TRAN(&CtrlUpdFinal19);       //update is not complete ,exit and try to update late
                    }
            }           
            return ret; 
        case CONTROL_UPD_TRANSPORT_LEVEL_ERROR_SIG: return Q_TRAN(&CtrlUpdReOpenConnection14); 
            ...
            for (;;)
               ...
               switch (state)
                  ...
                  case fw_update_start: 
                     ...
                     return Q_TRAN(&CtrlUpdFwStart17);//  //config not changed,config.fwu==1
                  case update_final:  return Q_TRAN(&CtrlUpdFinal19); //config not changed,config.fwu==0
...
QState CtrlUpdConf15(CONTROL * const me, QEvt const * const e)
{
	static uint8_t error_timeout_counter;
    static enum
        {
        waiting_tcp_data,
        check_packet_len,
        ...
        case CONTROL_TCP_DATA_SIG:
            {
                ...
                for (;;)
                    ...
                    switch (state)
                        ...
                        case check_packet_len:
                            {
                                pack_len=((XTDevPacketType*)pe->ptr)->head.len;
                                if ((pack_len==pe->data.size)&&(pack_len>sizeof(((XTDevPacketType*)pe->ptr)->head)))
                                    {
                                    state=check_packet_crc; 
                                    }
                                else
                        ...
                        case check_packet_crc:
                            {
                                uint16_t pack_crc=((XTDevPacketType*)pe->ptr)->head.crc;
                                ((XTDevPacketType*)pe->ptr)->head.crc=(uint16_t)0;
                                uint16_t calc_crc= MakeCRC16(pack_ptr,pack_len);
                                if (pack_crc==calc_crc)
                                    {
                                    state=parse_config;
                        ...
                        case parse_config:
                            {
                                FlagsType flag={0};
                                flag.save_config=0;
                                next_cmd_ptr=(char*)((XTDevPacketType*)pe->ptr)->payload;
                                uint32_t payload_size,config_size;
                                payload_size=pack_len-sizeof(XTDevPacketHeadType);//40
                                config_size=strlen(next_cmd_ptr)+1;
                                if (NULL==next_cmd_ptr) state=error_exit;
                                else if (config_size>payload_size)
                                    {
                                    OutDebugControl("CONFIG STRING ERROR!!!");
                                    //  OutDebugControlSprintf2("payload_size="," config_size=",payload_size,config_size);
                                    ...
                                    {
                                    OutDebugControl("CONFIG PASSWORD_ERROR!!!");
                                    state=error_exit; 
                                    }
                                else
                                    {
                                    ...
                                    if (1==flag.save_config)//save config
                                        {
                                        //DataPtrEvt *pe = Q_NEW(DataPtrEvt,DISK_SAVE_SIG);
                                        //pe->data.flag.restart_modem=flag.restart_modem;
                                        QACTIVE_POST(AO_Disk, &DiskSaveConfigEvt, me);
                                        QACTIVE_POST(AO_Disk, &DiskReadConfigEvt, me);
                                        state=waiting_config_write_ok;
                                        }
                                    else   state=exit;//save nothing
                                    }
                            }
                            break;
                        case waiting_config_write_ok: return Q_HANDLED();//waiting   CONFIG_WRITE_OK_SIG
                            //EXIT POINTS 
                        case error_exit: return Q_TRAN(&CtrlUpdReOpenConnection14);//any ERROR
                        case exit: return Q_TRAN(&CtrlUpdSupervisor16);                             
                        }
                    }            
            }
												case TIC_100ms_SIG:
												{
													if(++error_timeout_counter==200)
													{
														 OutDebugControl("CONFIG TIMEOUT ERROR!!!");
														return Q_TRAN(&CtrlUpdFinal19); //
													}
												}
												return Q_HANDLED();
...
QState CtrlUpdFwStart17(CONTROL * const me, QEvt const * const e)
{
    ...
    switch (e->sig)
        ...
        case CONTROL_TCP_DATA_SIG:
            {
                ...
                for (;;)
                    ...
                    switch (state)
                        ...
                        case waiting_tcp_data:
                            {
                                ...
                                else if (me->pack.size>MAX_FWU_PACKET_SIZE ||me->pack.size <=sizeof(XTDevPacketHeadType))
                                    {
                                    OutDebugControl("ERROR! PACKET TOO LARGE");
                                    }
                                else if (me->pack.size <sizeof(XTDevPacketHeadType))
                                    {
                                    OutDebugControl("ERROR! PACKET TOO LITTLE");
                                    }
                                else
                        ...
                        case check_packet_len:
                            {
                                if (((XTDevPacketType*)fwu_buf)->head.len!=me->pack.size)
                                    {
                                    OutDebugControl("LEN ERROR");
                                    state=error_exit;
                                    }
                        ...
                        case check_packet_crc:
                            {
                                state=error_exit;
                                uint16_t u16temp=((XTDevPacketType*)fwu_buf)->head.crc;
                                ((XTDevPacketType*)fwu_buf)->head.crc=(uint16_t)0;
                                if (u16temp!=MakeCRC16(fwu_buf,me->pack.size))
                                    {
                                    OutDebugControl("CRC ERROR");
                                    }
                                    ...
                                    {
                                    payload_ptr= ((XTDevPacketType*)fwu_buf)->payload;
                                    u16temp= ((XTDevFwDescriptorPayloadType*)payload_ptr)->pack_index;
                                    if (u16temp!=me->fw.pack_index)
                                        {
                                        OutDebugControl("pack_index ERROR");
                                        }
                        ...
                        case process_fw_descriptor:
                            {
                                XTDevFwDescriptorPayloadType *fwd_ptr=(XTDevFwDescriptorPayloadType*)payload_ptr;
                                me->fw.size=fwd_ptr->size;
                                me->fw.crc=fwd_ptr->crc;
                                me->fw.last_pack_index =fwd_ptr->max_pack_index;
                                me->fw.ver=fwd_ptr->ver;
                                ...
                                OutDebugControlSprintf2("pack_index=","max_pack_index=",me->fw.pack_index,me->fw.last_pack_index);
                                OutDebugControlSprintf2("fw_crc=","fw_size=",me->fw.crc,me->fw.size);
                                OutDebugControlSprintf2("fw_ver=","fw_addr=",me->fw.ver,me->fw.start_addr);
                                me->fw.pack_index++;
                                state=exit;
                        ...
                        case error_exit: return Q_TRAN(&CtrlUpdReOpenConnection14);
                        case exit:  return Q_TRAN(&CtrlUpdSupervisor16); 
                        }
                    }
            } 
        case CONTROL_UPD_PACKET_LEVEL_ERROR_SIG:
            {
                OutDebugControl("CONTROL_UPD_PACKET_LEVEL_ERROR_SIG");
            }
            return Q_TRAN(&CtrlUpdReOpenConnection14);
        case CONTROL_UPD_TRANSPORT_LEVEL_ERROR_SIG:
            {
                OutDebugControl("CONTROL_UPD_TRANSPORT_LEVEL_ERROR_SIG");
            }
            return Q_TRAN(&CtrlUpdReOpenConnection14);
        case Q_EXIT_SIG: return Q_HANDLED();
        }
        ...
        case CONTROL_TCP_DATA_SIG:
            {
                ...
                for (;;)
                    {
                    switch (ufw_cont_state)
                        {
                        case waiting_packet:
                            {
                                ufw_cont_state=bad_packet;
                                uint8_t *pack_ptr=(uint8_t*)((DataPtrEvt*)e)->ptr;
                                me->pack.size=((DataPtrEvt*)e)->data.size;
                                OutDebugControlSprintf2("PACKET size=","head=",me->pack.size,*pack_ptr);
                                if (((DataPtrEvt*)e)->data.size==1)
                                    ...
                                    {
                                    for (uint32_t i=0;i<me->pack.size;i++)fwu_buf[i]=*pack_ptr++;
                                    ufw_cont_state=check_packet_len;
                                    }
                            }
                            break;
                        case check_packet_len:
                            {
                                if (((XTDevPacketType*)fwu_buf)->head.len!=me->pack.size)
                                    {
                                    OutDebugControl("LEN ERROR");
                                    me->pack_remain_len=((XTDevPacketType*)fwu_buf)->head.len-me->pack.size;
                                    ufw_cont_state=remain_packet_request;
                                    }
                                else ufw_cont_state=check_packet_crc;   
                            }
                            break;
                        case check_packet_crc:
                            {
                                ...
                                if (u16temp!=MakeCRC16(fwu_buf,me->pack.size))
                                    ...
                                    else ufw_cont_state=process_fw_data;
                                    }                      
                            }
                            break;
                        case process_fw_data:
                            {
                                XTDevFwDataType *fw_ptr=(XTDevFwDataType*)payload_ptr;
                                uint16_t fw_ver=fw_ptr->ver;
                                uint16_t pack_index=fw_ptr->pack_index;
                                uint8_t *fw_data_ptr=fw_ptr->fw_data;
                                uint16_t fw_data_size=me->pack.size-(sizeof(XTDevPacketHeadType)+sizeof(fw_ptr->pack_index)+sizeof(fw_ptr->ver));
                                        ...
                                        case FW_OK:
                                            {
                                                ...
                                                ufw_cont_state=next_packet_request; 
                                            }
                                            break;
                                        case FW_END:
                                            {
                        ...
                        case next_packet_request:
                            {
                                me->bad_packet_counter=0;
                                if (++me->fw.pack_index>me->fw.last_pack_index)ufw_cont_state=exit_error;
                                else
                                    {
                                    DataPtrEvt *pe = Q_NEW(DataPtrEvt,MODEM_UPD_SEND_PACKET_SIG);
                                    pe->data.size= MakeFwReqPack(send_pack_buf, me->fw.pack_index,me->fw.ver);
                        ...
                        case bad_packet:
                            {
                                if (++me->bad_packet_counter>7)ufw_cont_state=exit_error;
                                else
                                    {
                                    DataPtrEvt *pe = Q_NEW(DataPtrEvt,MODEM_UPD_SEND_PACKET_SIG);
                                    pe->data.size= MakeFwReqPack(send_pack_buf, me->fw.pack_index,me->fw.ver);
                        ...
                        case remain_packet_request:
                            {
                                ...
                                ufw_cont_state=waiting_packet_remain;
                            }
                            return Q_HANDLED();
                        ...
                        case write_boot_rec:
                            {
                                ...
                                if (flash_status == FLASH_COMPLETE)
                                    ...
                                    ufw_cont_state=exit_write_boot_rec_ok;
                                    }
                                else
                                    {
                            ...
                            break;
                        case exit_error: return Q_TRAN(&CtrlUpdReOpenConnection14); 
                        case exit_write_boot_rec_ok: return Q_TRAN(&CtrlUpdSupervisor16);
                        }
                    }
            } 
        case CONTROL_UPD_PACKET_LEVEL_ERROR_SIG:
            {
                OutDebugControl("CONTROL_UPD_PACKET_LEVEL_ERROR_SIG!!!");
                OutDebugControlSprintf1("bad_packet_counter=",me->bad_packet_counter);               
                if (++me->bad_packet_counter>10) return Q_TRAN(&CtrlUpdReOpenConnection14); //return Q_TRAN(&CtrlUpdFinal19);
                switch (ufw_cont_state)
                    {
                    case waiting_packet:
                        {
        ...
        case Q_ENTRY_SIG: 
            {
                ...
                QACTIVE_POST(AO_Control, &ControlUpdateEndEvt, me); 
            }
            return Q_HANDLED();
        }
    return Q_SUPER(&CtrlUpdSuper12);
...
QState CtrlPackSuper3(CONTROL* const me, QEvt const * const e)
{
    switch (e->sig)
        ...
        case TIC_100ms_SIG:
            {  
							 if (++me->conf_tick_counter>600)//every min
							    {
										int save=0;
								   me->conf_tick_counter=0;
										 if (conf.log.regular_restart_counter <conf.secur.regular_restart_timeout)
										 {
											 conf.log.regular_restart_counter++;
											 save=1;
										 }
										 else
										 {
											 conf.log.regular_restart_counter=0;
											  conf.log.save=1;
                        QACTIVE_POST(AO_Disk,&DiskSaveConfigEvt, me);
                        return Q_TRAN(&CtrlSysRebootSuper22);											 
										 }
                   if (conf.log.updTryCounter>0)//config update allowed 
                    {
                    if (conf.log.updTryTimeout<CONFIG_UPDATE_TIMEOUT)//increment tryTimeout allowed
                        {
                            conf.log.updTryTimeout++;
													  save=1; 
                        }
                    }
										if(save==1)
...
{
debug_counter=0;
	OutDebugControlSprintf3("send_enable=","sendInterval_counter=","conf.packet_send.time=",me->send_enable,me->sendInterval_counter,conf.packet_send.time);  
}	
            }
            return Q_HANDLED();
        case Q_INIT_SIG:                      return Q_TRAN(&CtrlPackIdle7);
        case CONTROL_REBOOT_SIG:              return Q_TRAN(&CtrlSysRebootSuper22);
        case CONTROL_UPDATE_START_SIG:        return Q_TRAN(&CtrlUpdSuper12);
        case CONTROL_FLASH_ERROR_SIG:         
        case CONTROL_FRAM_ERROR_SIG:          return Q_TRAN(&CtrlMemoryErrSuper11); 
        case Q_EXIT_SIG:                      return Q_HANDLED();
        }
...
QState CtrlPackFinal6(CONTROL * const me, QEvt const * const e)
{
    switch (e->sig)
        ...
        case Q_ENTRY_SIG: 
            {
                AddStateHistory(6);
                QACTIVE_POST(AO_Control, &ControlUpdateStartEvt, me); 
            }
            return Q_HANDLED();
        }
...
QState CtrlPackIdle7(CONTROL * const me, QEvt const * const e)
{
    ...
    switch (e->sig)
        ...
        case TIC_100ms_SIG:
            { 
                if (1==CheckConfigUpdTimeout())
                    {
                    ret= Q_TRAN(&CtrlPackFinal6);  
                    }
                else if ((me->send_enable==1)&&DiskIsNotEmpty())
                    {
...
/*QState CtrlUpdSysRestart20(CONTROL * const me, QEvt const * const e)
{
...
QState CtrlMemoryErrSuper11(CONTROL * const me, QEvt const * const e)
{
    static uint8_t object_counter;
    static uint16_t restart_counter;
    static enum
        ...
        case Q_ENTRY_SIG: 
            {
                AddStateHistory(11); 
                restart_counter=600; 
               // QACTIVE_POST(AO_Modem,&ModemSmsSendEvt, me);
                state=send_sms;
            }
            return Q_HANDLED();
        case TIC_100ms_SIG:
            {
                switch (state)
                    ...
                    case waiting_objects:
                        {
													if(restart_counter)restart_counter--;
            ...
            return Q_HANDLED();
				case CONTROL_ERROR_SMS_PROCESSED_SIG:
						{
							if( state==send_sms)restart_counter=1;
						}
						return Q_HANDLED();
        case CONTROL_MODEM_IS_OFF_SIG:
            {
...
int DiskIsNotEmpty(void)
{
    if (flash_disk_image.count>0)return 1;
    if (FramRingBuf_size_used()>0)return 1;
    return 0;
}
...
void OutDebugControlSprintf2( char const *str1,char const *str2,uint32_t val1,uint32_t val2)
{
 ...
 void OutDebugControlSprintf3( char const *str1,char const *str2,char const *str3,uint32_t val1,uint32_t val2,uint32_t val3)
   {
    QS_BEGIN(QS_CONTROL, AO_Control)                                  
   QS_STR(str1); 
   QS_U32(4, val1);
...
FLASH_Status ErasePage(uint32_t Page_Address)
{
  ...
  status = FLASH_WaitForLastOperation(0xFFF);
  debug_erase_page=1;
  if(status == FLASH_COMPLETE)
  { 
    /* if the previous operation is completed, proceed to erase the page */
    FLASH->CR|=  ((uint32_t)0x00000002);
    FLASH->AR = Page_Address; 
    FLASH->CR|=  ((uint32_t)0x00000040);
...
uint8_t dst;
uint32_t word_counter,page_counter,byte_counter;
 enum
        {
        start,
        write_next_data,                    
...
FlashWriteResultType FlashWrite(CONTROL* const me,uint8_t *data_buf,uint32_t data_size)
{
    ...
    if (me->fw.pack_index==1)fw_state=start;
    switch (fw_state)
        {
        case start:
            {
							 start_val=tick_counter;
                ...
                if (fs!=FLASH_COMPLETE)
                    ...
                    switch (fs)
                        ...
                        case FLASH_ERROR_PG: return FW_FLASH_PROGRAM_ERROR; 
        ...
        case write_next_data:
            {
                ...
                FLASH_Lock(); 
										...
										dst=4;
									end_val=tick_counter;
										complete_time=end_val-start_val;
										end_val=start_val=0;
                if (fs == FLASH_COMPLETE)
                    {
...
int CheckConfigUpdTimeout(void)
{
    static uint8_t debug_counter=0;
    if (++debug_counter==50)
        {
        debug_counter=0;
        OutDebugControlSprintf2( "ConfigtryCounter=","ConfigtryTimeout=",conf.log.updTryCounter,conf.log.updTryTimeout);
        }
    return(conf.log.updTryCounter>0 && conf.log.updTryTimeout>=CONFIG_UPDATE_TIMEOUT)?1:0;
}

void UpdateOff(CONTROL* const me)
{
    conf.log.updTryCounter=0;
    conf.log.updTryTimeout=0;
    conf.log.save=1;
    QACTIVE_POST(AO_Disk,&DiskSaveConfigEvt, me);
    OutDebugControl("SEND DiskSaveConfigEvt");
}

void UpdateDefer(CONTROL* const me)
{
    conf.log.updTryTimeout=0;
    conf.log.save=1;
    QACTIVE_POST(AO_Disk,&DiskSaveConfigEvt, me);
...
void UpdateOn(CONTROL* const me)
{
    conf.log.updTryCounter=10;
...
void SendEnable(CONTROL* const me)
{
	static uint8_t tick_counter=0;
 if (flash_disk_image.count!=0) me->send_enable= 1;
 else if (++tick_counter==10)
          {
           tick_counter=0;
           if (me->send_enable==0)
               {
                if (++me->sendInterval_counter>conf.packet_send.time)me->send_enable= 1;
                }
          }
}
